...
- `LocalProxyServer.recycleListener()` — rebinds the accept socket only via the extracted `bindListener(...)` helper. Falls through to `start()` if there's no prior listener (cold-start safety).
  ...
  > **Correction (2026-08-16).** As shipped, this bound the replacement socket *before* closing the old one, on the stated premise that "SO_REUSEADDR makes simultaneous bind to the same port valid across the close". That premise is false on Darwin: `SO_REUSEADDR` does not permit a second bind over a socket that is already `LISTEN`ing — only `SO_REUSEPORT` does, and the project deliberately never sets it (it would let any process able to bind the port join the accept set and take a share of connections carrying `Proxy-Authorization`). The step therefore *never succeeded* against a live listener; it spent the full ten-second bind retry budget on `EADDRINUSE` against its own socket and then threw a raw NIO `IOError` at the user. Nothing caught it because every `LocalProxyServer` test bound `localPort = 0`, where each rebind lands on a fresh ephemeral port.
  >
  > `recycleListener()` now no-ops when the accept socket is healthy and correctly addressed, and closes-then-binds only when it is dead or bound to the wrong address — the cases a recycle can actually repair. Declining to drop a working listener also closes the window that let a co-resident corporate proxy agent take port 3128 during recovery, after which no restart could bind again. Covered by `Tests/ConduitTests/ListenerRecycleTests.swift`, which pins a fixed port.
