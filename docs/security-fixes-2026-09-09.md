...
Missing runtime configuration is accepted only for a genuine first run. Existing platform/preferences files or a platform journal identify established app/daemon state; runtime-only loaders also recognize prior snapshots, events, or readiness files. Deleting `config.json` while that state remains now rejects startup. Use explicit `--minimal` for a file-free headless run in a reused state directory. `pm-dns` produces no state markers, so every normal launch requires configuration; `--minimal` explicitly selects generic defaults. `pm-tunnel` also requires configuration, since defaults contain no tunnels.
