...
Devin also found that a nonpositive connection limit supplied by live GUI edits could trigger an assertion. Admission now rejects those peers through the existing structured rejection path, preserving active reservations and recovering when the limit becomes positive. Embedded tests cover zero, negative, and minimum-integer limits; the live scenario covers zero/negative edits while a tunnel remains active.

The two additional review flags describe intentional boundaries: file retention keeps only the current bounded generation, as documented above, and the SOCKS deadline bounds greeting/request parsing. PAC evaluation and connection setup retain their separate timeouts after parsing; admission remains reserved until channel closure throughout those phases.
