...
| `connection.pool_exhausted` | The connection pool refused a request at its `maxConnections` cap and the client got a 502 (`target=`, `reason=`). Emitted before the error log line. |
