...
| `lifecycle.superseded` | A start or stop stopped short after its platform work returned, because a later start or stop had begun in the meantime (`reason=superseded`) or, for a proxy start, the listeners it started were down by then (`reason=runtime_down`). The later operation owns the surfaces; its work was queued behind this one's. `detail: operation=proxy_start\|proxy_stop\|dns_start\|dns_stop\|runtime_start\|runtime_stop generation=… current=… reason=…`. |
