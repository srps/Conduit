...
- Audit: one unified-log line per connection with pid (diagnostic only), signing identifier, cdhash prefix, runtime flag, identity verdict, command and outcome. Successful pings go to the `info` level, which is not persisted by default.
