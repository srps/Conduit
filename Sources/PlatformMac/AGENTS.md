...
macOS glue: Keychain, `networksetup`, `SMAppService`, the SCDynamicStore VPN observer, `NWPathMonitor`, the helper XPC client, the `/etc/resolver` writer, and the lifecycle policy both hosts share (`RuntimeReconciler`, `PlatformIntegrationReconciler`, `SplitDNSVPNGate`, `PlatformStateJournal`).
