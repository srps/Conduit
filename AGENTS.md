...
| Standalone DNS forwarder       | `swift run pm-dns --minimal --port 5353 --verbose`                                           | Local DoH forwarder, no proxy                                                   |
| App bundle (debug)             | `./bundle-app.sh`                                                                  | Builds `Conduit.app` in-tree                                               |
| Dev instance beside the installed app | `open -n -a "$PWD/Conduit.app" --args --dev [--section dns] [--vpn utun4] [--upstream host:port] [--dev-state-dir PATH]` | Debug builds only. Runs over the harness's `FakeMachine` with a scratch state directory under `$TMPDIR/conduit-dev` and every port ephemeral, and shows the popover in a plain panel for screenshots and the accessibility inspector. `open -n` starts a new instance rather than activating the installed app; a direct executable launch from an agent session has no window server access. The state directory's `proxy.log` confirms the windows came up. Never quit the installed app, and never run `--install` while it is serving |
...
- `Sources/PlatformMac/` - macOS-specific glue: Keychain, networksetup wrappers, SMAppService, SCDynamicStore VPN observer, NWPathMonitor, helper-XPC client, `/etc/resolver` writer, and the lifecycle policy both runtime hosts share (`RuntimeReconciler`, `PlatformIntegrationReconciler`, `SplitDNSVPNGate`, `PlatformStateJournal`). Also the fakes the hosts are tested and run against, in `PlatformFakes.swift` (`FakeMachine`, `RecordingPrivilegeClient`, `FakeLoginItems`, `FakeHelperLifecycle`, `InMemorySecretStore`); they live in Sources, not Tests, because the app's `--dev` mode is an executable and SwiftPM lets only a test target import a test target. Depends on `ProxyKernel` + `ConduitShared`.
- `Sources/Conduit/` - SwiftUI app and `AppState` orchestrator. Views stay thin. A change to the lifecycle composition in `AppState` (start/stop, ownership guards, quit, VPN handling) adds a scenario to `Tests/ConduitTests/AppStateHarnessTests.swift`, which runs a real orchestrator over the `FakeMachine` in `Sources/PlatformMac/PlatformFakes.swift` and asserts on the machine and the journal. `App/DevLaunch.swift` builds the same composition for `Conduit --dev` (debug builds only), so the injected platform/persistence boundaries are shared. Notification and network-path services are still real; see `docs/architecture.md` for the dev mode's limits. Its windows are presented from AppKit on purpose: a `Window` scene with `defaultLaunchBehavior(.presented)` never came up for an instance launched from a terminal or `open -n`, `SceneBuilder` takes only `if #available` conditionals, and a `MenuBarExtra` label wider than its glyph is dropped beside a notch; the comments there say what was tried.
- `Sources/ConduitDaemon/` - the daemon runtime host, `AppState`'s twin over the same managers and reconciler. A lifecycle rule added to one host lands in the other, with the twin scenario in `Tests/ConduitTests/DaemonRuntimeHostTests.swift` over the same `FakeMachine`; where the daemon cannot express it yet, a strict `XCTExpectFailure` names what the migration still has to bring over.
...
- **Never quit, kill or replace the running Conduit to test a build.** It is serving the machine's proxy, and tearing it down mid-session is the side effect the fakes exist to avoid. Use the dev instance from the toolchain table. `bundle-app.sh --install` quits the running app and then kills it by process name before copying; nothing automated runs it while Conduit is serving.
...
- **Always give a new `AppState` collaborator that touches the machine a seam and a fake.** The seam is an `init` parameter with the production default, next to `runtimeEnvironment`, `privilegeClient`, `helperLifecycle`, `credentialStore`, `commandRunner`, `homeDirectory`, `resolverDirectory`, `loginItemManager` and `vpnStatusMonitor`; the fake goes in `Sources/PlatformMac/PlatformFakes.swift`; and both `AppStateHarness.launch()` and `DevLaunch.makeAppState` inject it. Existing notification and network-path collaborators still lack these seams; track that gap in `docs/refactor-notes-2026-09-09.md`. Review of #24 found two collaborators without one, the helper lifecycle and the Keychain, and through them a dev instance could replace the installed helper and delete the installed app's credentials while its log said nothing reached the system.
